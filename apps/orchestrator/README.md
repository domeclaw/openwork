...
Host orchestrator for opencode + THWork server + opencode-router. This is a CLI-first way to run host mode without the desktop UI.
...
When `OPENWORK_DEV_MODE=1` is set, orchestrator uses an isolated OpenCode dev state for config, auth, data, cache, and state. THWork's repo-level `pnpm dev` commands enable this automatically so local development does not reuse your personal OpenCode environment.
...
THWork URL and a redacted `opencode attach` command, while keeping live credentials out of the detached summary.
...
`openwork` emits a unified log stream from OpenCode, THWork server, and opencode-router. Use JSON format for
...
THWork server logs every request with method, path, status, and duration. Disable this when running
...
- Use the **THWork connect URL** and **client token** to connect a remote THWork client.
- The THWork server advertises the **OpenCode connect URL** plus optional basic auth credentials to the client.
