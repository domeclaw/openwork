 ...
 *
 * Every workspace-scoped API call in the app must route to the THWork server
 * that actually owns that workspace. For local workspaces that's the user's
 * local THWork server. For workspaces hosted on a remote THWork worker
...
export type ResolvedWorkspaceEndpoint = {
  /** Host URL of the THWork server that owns this workspace (no `/workspace` mount). */
  baseUrl: string;
  ...
  workspaceId: string;
  /** True when the workspace lives on a remote THWork worker, not the user's local server. */
  isRemote: boolean;
